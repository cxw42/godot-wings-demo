...
func rebuild_():
    ...
    if false: # XXX experiment
        ...
        var a_bottom = a_beam.global_position - a_beam.global_basis.y.normalized() * Vector3(0,a_m/2,0)
        #a_to_b.global_position = a_bottom
        b_beam.global_position = a_bottom + b_beam.global_basis.y.normalized() * Vector3(0, b_m/2, 0)
